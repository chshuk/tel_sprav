...
def copy_file(number_str, file_from, file_to):
    res = read_file(file_from)
    print(res[number_str])
    copy_str = [value for key, value in res[number_str].items()]
    print(copy_str)
    write_file(file_to, copy_str)
    return
...
def main():
    while True:
        ...
        if command == 'q':
            ...
            number_line  = int(input("Введите номер строки для копирования: ")) 
            if not exists(file_copy):
                create_file(file_copy)
            copy_file(number_line, file_name, file_copy)
